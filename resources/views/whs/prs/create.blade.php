...
@section('content')
    <div class="row">
        <form method="POST" action="{{ route('whs.prs.store') }}" class="form-horizontal" id="prFormSubmit">
            @csrf
            <div class="ibox">
                <div class="ibox-title">
                    <h3>สร้างใบขอซื้อ (Purchase Requisition)</h3>
                    <button type="button" class="btn btn-primary" onclick="prFormSubmit()" id="submitButton">บันทึก</button>
                    <a href="{{ route('whs.prs.index') }}" class="btn btn-warning waves-effect">ยกเลิก</a>
                </div>
    ...
    <table style="display: none;">
        <tbody>
        <tr class="addTr">
            <td><span class="good_code"></span><input type="hidden" name="good_id[]" value="" class="good_id"></td>
            <td>
            ...
            <td><span class="good_unit"></span></td>
            <td><span class="amount_in_warehouse"></span></td>
            <td>
                <center><button type="button" name="button" class="btn btn-danger deleteRow">ลบ</button>
                </center>
            </td>
        </tr>
        </tbody>
